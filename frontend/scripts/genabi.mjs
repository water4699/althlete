...
if (deployLocalhost && deploySepolia) {
  // Only check ABI compatibility if Sepolia deployment has ABI
  if (deploySepolia.abi && JSON.stringify(deployLocalhost.abi) !== JSON.stringify(deploySepolia.abi)) {
    console.error(
    ...
    process.exit(1);
  } else if (!deploySepolia.abi) {
    console.warn(
      `${line}Sepolia deployment found but no ABI available. Using localhost ABI for Sepolia network.${line}`
    );
  }
}
